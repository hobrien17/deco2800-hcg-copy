...
/**
 * UI for Multiplayer Lobby. Made by Leo Zheng from Team 9
 */
public class LobbyContext extends UIContext {
	...
	public LobbyContext() {
		...
		ContextManager contextManager = (ContextManager) gameManager
				.getManager(ContextManager.class);
		NetworkManager networkManager = (NetworkManager) gameManager
				.getManager(NetworkManager.class);
		PlayerManager playerManager = (PlayerManager) gameManager
				.getManager(PlayerManager.class);
		TextureManager textureManager = (TextureManager) gameManager
		...
		main.setBackground(
				new Image(textureManager.getTexture("multi_menu_background"))
						.getDrawable());
		...
		playerPortrait1 = new Image(
				textureManager.getTexture("lobby_image_frame"));
		playerPortrait2 = new Image(
				textureManager.getTexture("lobby_image_frame"));
		playerPortrait3 = new Image(
				textureManager.getTexture("lobby_image_frame"));
		playerPortrait4 = new Image(
				textureManager.getTexture("lobby_image_frame"));
		// buttons set up
		back = new ImageButton(
				new Image(textureManager.getTexture("lobby_back_button"))
						.getDrawable());
		start = new ImageButton(
				new Image(textureManager.getTexture("lobby_start_button"))
						.getDrawable());
		send = new ImageButton(
				new Image(textureManager.getTexture("lobby_send_button"))
						.getDrawable());
		readyCheckBox = new CheckBox("Ready", skin);
		// label initialize
		lobbyLabel = new Label("Lobby Name: " + networkManager.getLobbyName(),
				skin);
		...
		hostNameAdd.addListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				lobbyLabel.setText(
						"Lobby Name: " + networkManager.getLobbyName());
				hostName.hide();
			}
		});
	...
	/**
	 * adds the new player's portrait into the current player stack. The 1st
	 * player will always be the host.
	 * 
	 * @param playerId
	 * @param playerImage
	 */
