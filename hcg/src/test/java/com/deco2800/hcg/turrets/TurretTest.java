...
public class TurretTest {
	...
	/*
	 * Tests that the ice turret freezes nearby enemies, and destroys itself
	 * when done
	 */
	@Test
	...
	/*
	 * Test that an explosion is successfully created (and removed when done)
	 */
	@Test(timeout = 1000)
	public void testExplosion() {
		...
		exp.onTick(0);
		assertEquals("The explosion should be at the correct x position", CORPSE_X + 1 - EXP_CHANGE, exp.getPosX(), 0);
		assertEquals("The explosion should be at the correct y position", CORPSE_Y, exp.getPosY(), 0);
		assertEquals("The explosion should have the correct new x-length", 0.01f + EXP_CHANGE, exp.getXRenderLength(),
				0);
		assertEquals("The explosion should have the correct new y-length", 0.01f + EXP_CHANGE, exp.getYRenderLength(),
				0);

		while (gm.getWorld().containsEntity(exp)) {
			exp.onTick(0);
			// this loop will continue until the explosion is destroyed
			// if the explosion is never destroyed, the test will timeout (after
			// 1 second)
		}
	}
	...
	public void testExplosion2() {
		...
		for (int j = 0; j < 2; j++) {
			...
			exp.onTick(0);
			assertFalse(gm.getWorld().containsEntity(exp));
		}
